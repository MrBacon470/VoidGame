...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../node_modules/break_eternity.js/dist/break_eternity.esm.js":[function(require,module,exports) {
"use strict";
...
var Decimal = /*#__PURE__*/function () {
  ...
  _createClass(Decimal, [{
    ...
    value: function tetrate() {
      ...
      var payload = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : FC_NN(1, 0, 1); //x^^1 == x
...
},{}],"TS/Data.ts":[function(require,module,exports) {
"use strict";
...
var break_eternity_js_1 = __importDefault(require("break_eternity.js"));

exports.D = function (x) {
  return new break_eternity_js_1.default(x);
}; //create all the variables in a globalData object for saving


function getDefaultObject() {
  return {
    void: [exports.D(10), exports.D(0), exports.D(0), exports.D(0)],
    voidUps: [[exports.D(0), exports.D(0), exports.D(0), exports.D(0)], [exports.D(0), exports.D(0), exports.D(0), exports.D(0)], [exports.D(0), exports.D(0), exports.D(0), exports.D(0)], [exports.D(0), exports.D(0), exports.D(0), exports.D(0)]],
    time: Date.now(),
    devSpeed: 1,
    hasTab: [false],
    currentTab: 0
...
function defaultTempVars() {
  return {
    voidGains: [exports.D(0), exports.D(0), exports.D(0), exports.D(0)],
    voidUpCosts: [[exports.D(10), exports.D(100), exports.D(1e3), exports.D(1e4)]]
  };
}

exports.globalTemp = defaultTempVars();
exports.globalData = getDefaultObject(); //saving and loading

var saveName = "voidGameSave";

function save() {
  window.localStorage.setItem(saveName, JSON.stringify(exports.globalData));
}

function load() {
  var savedata = JSON.parse(window.localStorage.getItem(saveName) || '{}');
  if (savedata !== undefined) fixSave(exports.globalData, savedata);
}

exports.load = load; //fix saves

function fixSave(main, data) {
  if (main === void 0) {
    main = getDefaultObject();
  }

  if (_typeof(data) === "object") {
    Object.keys(data).forEach(function (i) {
      if (main[i] instanceof break_eternity_js_1.default) {
        main[i] = exports.D(data[i] !== null ? data[i] : main[i]);
      } else if (_typeof(main[i]) == "object") {
...
function exportSave() {
  save();
  var exportedData = btoa(JSON.stringify(exports.globalData));
  var exportedDataText = document.createElement("textarea");
...
function importSave() {
  var importedData = prompt("Paste your save data here!"); // @ts-ignore

  exports.globalData = Object.assign(getDefaultObject(), JSON.parse(atob(importedData)));
  save();
  location.reload();
}
...
},{"break_eternity.js":"../node_modules/break_eternity.js/dist/break_eternity.esm.js"}],"TS/Cache.ts":[function(require,module,exports) {
"use strict";
...
exports.DOMCache = new Map();

exports.DOMCacheGetOrSet = function (id) {
  var cachedEl = exports.DOMCache.get(id);
  if (cachedEl) return cachedEl;
  var el = document.getElementById(id);
  exports.DOMCache.set(id, el);
  return el;
};
},{}],"TS/data.ts":[function(require,module,exports) {
"use strict";
...
},{"break_eternity.js":"../node_modules/break_eternity.js/dist/break_eternity.esm.js"}],"TS/Formatting.ts":[function(require,module,exports) {
"use strict";
...
function exponentialFormat(num, precision, mantissa) {
  ...
  var m = num.div(break_eternity_js_1.default.pow(10, e));

  if (m.toStringWithDecimalPlaces(precision) === 10) {
    m = new break_eternity_js_1.default(1);
    e = e.add(1);
  }

  e = e.gte(1e9) ? format(e, 3) : e.gte(10000) ? commaFormat(e, 0) : e.toStringWithDecimalPlaces(0);
  if (mantissa) return m.toStringWithDecimalPlaces(precision) + "e" + e;else return "e" + e;
}

function commaFormat(num, precision) {
  if (num === null || num === undefined) return "NaN";
  if (num.mag < 0.001) return 0 .toFixed(precision);
  var init = num.toStringWithDecimalPlaces(precision);
  var portions = init.split(".");
...
function format(decimal, precision) {
  if (precision === void 0) {
    precision = 2;
  }

  decimal = new break_eternity_js_1.default(decimal);
  ...
  if (decimal.gte("eeee1000")) {
    ...
    if (slog.gte(1e6)) return "F" + format(slog.floor());else return break_eternity_js_1.default.pow(10, slog.sub(slog.floor())).toStringWithDecimalPlaces(3) + "F" + commaFormat(slog.floor(), 0);
  } else if (decimal.gte("1e1000000")) return exponentialFormat(decimal, 0, false);else if (decimal.gte("1e10000")) return exponentialFormat(decimal, 0);else if (decimal.gte(1e6)) return exponentialFormat(decimal, precision);else if (decimal.gte(1e3)) return commaFormat(decimal, 0);else if (decimal.gte(0.0001)) return regularFormat(decimal, precision);else if (decimal.eq(0)) return 0 .toFixed(precision);
...
exports.formatWhole = formatWhole;

function formatTime(s) {
  if (s < 60) return format(s) + "s";else if (s < 3600) return formatWhole(Math.floor(s / 60)) + "m " + format(s % 60) + "s";else if (s < 86400) return formatWhole(Math.floor(s / 3600)) + "h " + formatWhole(Math.floor(s / 60) % 60) + "m " + format(s % 60) + "s";else if (s < 31536000) return formatWhole(Math.floor(s / 86400) % 365) + "d " + formatWhole(Math.floor(s / 3600) % 24) + "h " + formatWhole(Math.floor(s / 60) % 60) + "m " + format(s % 60) + "s";else return formatWhole(Math.floor(s / 31536000)) + "y " + formatWhole(Math.floor(s / 86400) % 365) + "d " + formatWhole(Math.floor(s / 3600) % 24) + "h " + formatWhole(Math.floor(s / 60) % 60) + "m " + format(s % 60) + "s";
}

exports.formatTime = formatTime;

function toPlaces(x, precision, maxAccepted) {
  x = new break_eternity_js_1.default(x);
  var result = x.toStringWithDecimalPlaces(precision);

  if (new break_eternity_js_1.default(result).gte(maxAccepted)) {
    result = new break_eternity_js_1.default(maxAccepted - Math.pow(0.1, precision)).toStringWithDecimalPlaces(precision);
  }
...
function invertOOM(x) {
  var e = x.log10().ceil();
  var m = x.div(break_eternity_js_1.default.pow(10, e));
  e = e.neg();
  x = new break_eternity_js_1.default(10).pow(e).times(m);
  return x;
}
},{"break_eternity.js":"../node_modules/break_eternity.js/dist/break_eternity.esm.js"}],"TS/Void.ts":[function(require,module,exports) {
"use strict";
...
function purchaseVoidUp(a, b) {
  switch (a) {
    case 'Regular':
      if (data_1.globalData.void[0].gte(data_1.globalTemp.voidUpCosts[0][b])) {
        data_1.globalData.void[0] = data_1.globalData.void[0].sub(data_1.globalTemp.voidUpCosts[0][b]);
        data_1.globalData.voidUps[0][b] = data_1.globalData.voidUps[0][b].plus(1);
      }

  }
}

exports.purchaseVoidUp = purchaseVoidUp;
},{"./data":"TS/data.ts"}],"TS/Listeners.ts":[function(require,module,exports) {
"use strict";
...
function generateEventHandlers() {
  var _loop_1 = function _loop_1(i) {
    document.getElementById("vu" + (i + 1)).addEventListener('click', function () {
      Void_1.purchaseVoidUp("Regular", i);
    });
  };
...
exports.generateEventHandlers = generateEventHandlers;
},{"./Void":"TS/Void.ts"}],"TS/main.ts":[function(require,module,exports) {
"use strict";
...
exports.greekLettersLower = ['α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ', 'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'τ', 'υ', 'φ', 'χ', 'ψ', 'ω'];
exports.greekLettersUpper = ['A', 'B', 'Γ', 'Δ', 'E', 'Z', 'H', 'Θ', 'Ι', 'K', 'Λ', 'M', 'N', 'Ξ', 'O', 'Π', 'P', 'Σ', 'T', 'Y', 'Φ', 'Χ', 'Ψ', 'Ω'];

function changeTab(tab) {
  var x = tab - 1;
  if (x > 0) Data_1.globalData.currentTab = Data_1.globalData.hasTab[x] ? tab : Data_1.globalData.currentTab;else if (x <= 0) Data_1.globalData.currentTab = tab;
}
...
function updateResourceGain() {
  for (var i = 0; i < 4; i++) {
    Data_1.globalTemp.voidGains[0] = Data_1.globalTemp.voidGains[0].plus(Data_1.globalData.voidUps[0][i]);
    Data_1.globalTemp.voidGains[0] = Data_1.globalTemp.voidGains[0].plus(Data_1.globalData.voidUps[0][i].times(10));
    Data_1.globalTemp.voidGains[0] = Data_1.globalTemp.voidGains[0].plus(Data_1.globalData.voidUps[0][i].times(100));
    Data_1.globalTemp.voidGains[0] = Data_1.globalTemp.voidGains[0].plus(Data_1.globalData.voidUps[0][i].times(1e3));
  }
}
...
window.onload = function () {
  Data_2.load();
  Listeners_1.generateEventHandlers();
};

window.setInterval(function () {
  mainLoop();
}, 50);
},{"./Data":"TS/Data.ts","./Update":"TS/Update.ts","./Listeners":"TS/Listeners.ts"}],"TS/Update.ts":[function(require,module,exports) {
"use strict";
...
function updateHTML() {
  ...
  Cache_1.DOMCacheGetOrSet('voidAmtText').innerHTML = Formatting_1.format(data_1.globalData.void[0]) + " | " + Formatting_1.format(data_1.globalTemp.voidGains[0]) + "/s"; //Tab Specific Updates

  if (data_1.globalData.currentTab === 0) {
    for (var i = 0; i < 4; i++) {
      Cache_1.DOMCacheGetOrSet("vu" + (i + 1)).innerHTML = "Void Upgrade " + main_1.greekLettersLower[i] + " - \n            Level: " + Formatting_1.format(data_1.globalData.voidUps[i][0]) + " | Cost: " + Formatting_1.format(data_1.globalTemp.voidUpCosts[0][i]) + " Voids";
    }
  } //Assorted Functions
...
},{"./Cache":"TS/Cache.ts","./data":"TS/data.ts","./Formatting":"TS/Formatting.ts","./main":"TS/main.ts"}],"TS/Main.ts":[function(require,module,exports) {
"use strict";
...
},{"./Data":"TS/Data.ts","./Update":"TS/Update.ts","./Listeners":"TS/Listeners.ts"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/Main.684d665f.js.map